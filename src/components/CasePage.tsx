...
export default function CasePage({data}: {data: any}) {
  return (
    <section className="relative">
      ...
      <Image src={"/images/planetas-novo.png"} alt="planetas" width={420} height={420} className="absolute top-[113px] left-0 lg:left-[32vw] mx-auto z-10" data-aos="fade-down" data-aos-delay="300" />
      <Header className="z-40" />
      <div className="fixed top-0 w-full h-[93px] bg-purple z-30 rounded-b-4xl" />

      <h1 className="sr-only">{data.product}</h1>

      <div className="container pt-80 pb-20 2xl:!px-20">
        <div className="font-neulis-regular text-xl uppercase" data-aos="fade-right" data-aos-delay="1000">
          <p>
            Praça: <span className="font-neulis-bold-italic text-[22px]">{data.plaza} </span> 
          </p>
          <p>
            Cliente: <span className="font-neulis-bold-italic text-[22px]">{data.client} </span>
          </p>
          <p>
            Produto: <span className="font-neulis-bold-italic text-[22px]">{data.product}</span>
          </p>
        </div>
        ...
        <section className="mt-32">
          <div className="relative" data-aos="fade-up">
            <Swiper 
              className="!h-[700px] bg-purpleLight"
              grabCursor={true}
              modules={[Navigation]}
              navigation={{
            ...
            >
              ...
              {data.carousel.map((item: any, index: number) => (
                <SwiperSlide className="relative w-full !h-[700px]" key={item.src}>
            ...
            <div className="absolute w-full flex justify-between top-1/2 -translate-y-[50%] z-20">
              <button className="group swiper-button-prev-case relative cursor-pointer lg:text-3xl lg:right-8 py-[15px] pl-[22px] pr-[24px] rounded-full bg-purpleDark hover:bg-purpleLight transition-all duration-300 disabled:opacity-0">
              ...
              <button className="group swiper-button-next-case relative cursor-pointer lg:text-3xl lg:left-8 py-[15px] pl-[24px] pr-[22px] rounded-full bg-purpleDark hover:bg-purpleLight transition-all duration-300 disabled:opacity-0">
          ...
          {data.bigImage && (
            <div className="mt-7 relative w-full !h-[700px] bg-purpleLight">
          ...
          {data.grid && (
            <div className="mt-7 grid grid-cols-1 lg:grid-cols-2 gap-1">
              {data.grid.map((item: any) => (
                <div key={item.src} className="relative w-full h-[500px] bg-purpleLight">
                  <Image src={item.src} alt={item.alt} fill quality={100} className="object-cover" />
                </div>
              ))}
            </div>
